...
node {
    ...
    def PACKAGE_NAME='0Ho2w000000CaXjCAK'
    def PACKAGE_VERSION
    ...
    withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
        ...
        stage('Generate Password Package Install Scratch Org') {
        if (isUnix()) {     
        ...
        if (rc != 0) {
                error 'Salesforce package generate password for scratch org  failed.'
            }
        }
        
        stage('Open Password generated Package Install Scratch Org') {
        if (isUnix()) {     
        rc = command "${toolbelt} force:org:open -u installorg"
        } else {
        rc = command "\"${toolbelt}\" force:org:open -u installorg"
        }
        
        if (rc != 0) {
                error 'Open Password generated Package Install scratch org  failed.'
            }
        }
    }
}
